...
(async () => {
  ...
  const grid = await (async ($parent, url) => {
    ...
    const filter = e => {
      //BUG질문 : myboard파일에서는 마지막에 펭수사진 옆에 그냥 빈칸인데
      //왜 여기서는 빈사진 나오면 에러가 날까요..?
      $el.lastElementChild.firstElementChild.innerHTML = '';
      let searchText = e.target.value;
      const resultList =
        timelineList.filter(x => x.text.includes(`${searchText}`)) ||
        timelineList.filter(x => x.name.includes(`${searchText}`));
    ...
    const sort = e => {
      ...
      if (e.target.id === 'newButton') {
        timelineList.sort((x, y) => {
          return (
            y.timestamp.replace(/\s|\/|:/gi, '') -
            x.timestamp.replace(/\s|\/|:/gi, '')
          );
        });
        const listList = divide(timelineList, ITEM_PER_ROW);
